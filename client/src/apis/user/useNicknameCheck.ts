...
export async function checkUserNickName(nickname: string): Promise<BaseResponse<boolean>> {
    try {

      console.log(nickname)
      const response = await instance.get<BaseResponse<boolean>>(`user/nickname/check?nickname=${nickname}`);
      
      if (response.data.status !== 200) {
        throw new Error(response.data.message || "닉네임 중복 확인 중 오류가 발생했습니다.");
      }
      ...
      try {
        ...
        if (data?.success) {
          ...
          toast[data.data ? 'success' : 'error'](
            data.data ? "이미 사용 중인 닉네임입니다." : "사용 가능한 닉네임입니다."
          );
        }
      } catch (error) {
